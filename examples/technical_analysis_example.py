...
def example_integration():
    ...
    # 创建增强技术因子
    enhanced_factor = EnhancedTechnicalFactor(weight=0.3)  # 技术因子权重30%
    
    # 模拟股票列表
    stocks = []
    for i in range(5):
        stock = StockInfo(
            code=f"STOCK{i+1:03d}",
            name=f"股票{i+1}",
            price=10.0 + i * 0.5,
            dividend_yield=3.0 + i * 0.2,
            pe_ratio=15.0 - i * 0.5,
            pb_ratio=2.0 - i * 0.1,
            ...
            eps=2.0 + i * 0.1,
            book_value=5.0 + i * 0.2,
            week_52_high=12.0 + i * 0.5,
            week_52_low=8.0 - i * 0.2
        )
        stocks.append(stock)
    ...
    for stock in stocks:
        # 模拟历史数据积累
        for _ in range(20):
            enhanced_factor.calculate(stock)
        
        score = enhanced_factor.calculate(stock)
        technical_result = enhanced_factor.get_technical_analysis_result(stock)
        
        print(f"{stock.code}: {score:.4f}")
        
        if technical_result:
            buy_signals = len(technical_result.signals.get('buy_signals', []))
            sell_signals = len(technical_result.signals.get('sell_signals', []))
